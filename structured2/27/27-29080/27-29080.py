...
for a in f:
    ...
    db1.append([float(ls[0]), float(ls[1]), ls[2]])
...
bg3 = max([dist(cen2, p) for p in db1 if fullmatch(r"L3\w*", p[2])])
bg31 = max([dist(cen1, p) for p in db1 if fullmatch(r"L3\w*", p[2])])
